...
import { IProductEntity, ICommentEntity, IProductSearchFilter, ProductCreatePayload, IProductImageEntity, ImagesRemovePayload, ProductAddImagesPayload } from "../../types";
...
import { INSERT_PRODUCT_QUERY,
         DELETE_IMAGES_QUERY,
         INSERT_PRODUCT_IMAGES_QUERY } from "../services/queries";
...
const throwServerError = (res: Response, e: Error) => {
  ...
  // получения списка товаров вместе с изображениями
...
// метод получения товара по id вместе с изображениями
  ...
  // метод добавления товара с добавлением изображений в соответствующую таблицу
...
// метод удаления товара с предварительным удалением всех изображений и комментариев,
...
productsRouter.post('/remove-images/:id', async (
  ...
  try {
    const [info] = await connection.query<ResultSetHeader>("DELETE FROM images WHERE product_id = ?", [req.params.id]);
  ...
  //  удаление коментапмев  конкретного продукта из таблицы comments
productsRouter.post('/remove-comments/:id', async (
  ...
  res.setHeader("Access-Control-Allow-Origin", "*");
  try {
    const [info] = await connection.query<ResultSetHeader>("DELETE FROM comments WHERE product_id = ?", [req.params.id]);
